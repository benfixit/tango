...
class LS {
  ...
  public static function forgotPassword(){
    ...
    if( !isset($_POST['logSysForgotPass']) && !isset($_GET['resetPassToken']) && !isset($_POST['logSysForgotPassChange']) ){
      $html = '<h2 align="center" style="margin-top: 100px;">Reser Password</h2>';
      $html .= '<form class="cmxform" id="resetForm" action="'. self::curPageURL() .'" method="POST" style="width: 300px; margin: 0 auto; margin-top: 10px;">';
        $html .= "<div class='form-group'>";
        $html .= "<label>{$identName}</label>";
          $html .= "<input type='text' class='form-control' id='logSysIdentification' placeholder='Enter your {$identName}' size='25' name='identification' />";
        $html .= "</div>";
        $html .= "<p><button name='logSysForgotPass' class='btn btn-default' type='submit'>Reset Password</button></p>";
      $html .= "</form>";
      echo $html;
      /**
   ...
   */
   ...
   /**
   * -------------------------
   * Start of Greyfield CRUD and other method Tools/Functions
   * -------------------------
   *  ADD THIS CODE IN YOUR METHOD TO SHOW ERROR
   *  self::$dbh->setAttribute( \PDO::ATTR_ERRMODE, \PDO::ERRMODE_EXCEPTION );
   */

  /**
   * A function to save properties details.
   */
  public static function saveArticle( $article_details = array() ){
    self::construct();
    if( count($article_details) == 0 ){
      /* If there is no article_details fields mentioned, make the default query */
      return "empty_array";
    }else{
      /* if there are article_details fields to add value to, make the query and bind values according to it */
      //remove videourl key and value from array.
      $keys   = array_keys($article_details);
      $columns = implode(",", $keys);
      $colVals = implode(",:", $keys);
      $sql   = self::$dbh->prepare("INSERT INTO `". self::$config['db']['article_table'] ."` ($columns) VALUES (:$colVals)");
      /* Bind the values */
 ...
 public static function allArticleCount(){
    $sql = self::$dbh->prepare("SELECT COUNT(*) as num_rows FROM `". self::$config['db']['article_table'] ."`");
    $sql->execute();
    $allNumRows = $sql->fetch(\PDO::FETCH_ASSOC);
    return $allNumRows[num_rows];
  }

  public static function loadAllArticle($page){
    $offset = $page * 10;
    $sql = self::$dbh->prepare("SELECT * FROM `". self::$config['db']['article_table'] ."` ORDER BY id DESC LIMIT $offset, 10;");
    $sql->execute();
    return $sql;
  }
  public static function loadArticle($identification){
    $sql = self::$dbh->prepare("SELECT * FROM `". self::$config['db']['article_table'] ."` WHERE `id` = :id");
    $sql->execute(array(
      ":id" => $identification
    ));
    return $sql;
  }
  public static function descArticle(){
    return $sql = self::$dbh->query("DESCRIBE `". self::$config['db']['article_table'] ."`");
  }

  public static function deleteArticle($articleId){
        // Verify data
      $sql = self::$dbh->prepare("DELETE FROM `". self::$config['db']['article_table'] ."` WHERE id = ?");
      ...
      if($sql){
          ...
          echo '<div class="statusmsg">Delete operation failed, please try again.</div>';
        }
    }
